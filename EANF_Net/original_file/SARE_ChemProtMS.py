...
print("model load")
# ############################################读取数据#################################################################
df_train = pd.read_csv('../data/ChemProtMS/train.tsv', sep='\t')
...
class AttentionWeightedMLPFusion(nn.Module):
    """
    注意力加权 + MLP融合模型
    结合注意力机制的动态权重分配和MLP的非线性融合能力
    """

    def __init__(self, model_name_1, model_name_2, model_name_3, num_labels=6,
                 hidden_dim=128, mlp_hidden_dim=256, num_heads=8, dropout_rate=0.3):
        super(AttentionWeightedMLPFusion, self).__init__()
        ...
        self.mlp_fusion = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(dropout_rate),
            nn.Linear(mlp_hidden_dim // 2, hidden_dim * 3),  # 输出融合后的特征
            nn.GELU()
        )
    ...
    def forward(self, input_ids_1, attention_mask_1, input_ids_2, attention_mask_2,
        ...
        Args:
            return_weights: 是否返回注意力权重用于分析
        """
        # 1. 特征提取
        feat1 = self.Bert_1(input_ids_1, attention_mask_1)[0][:, 0]  # [batch, 768]
        feat2 = self.Bert_2(input_ids_2, attention_mask_2)[0][:, 0]  # [batch, 768]
        feat3 = self.Bert_3(input_ids_3, attention_mask_3)[0][:, 0]  # [batch, 768]

        # 2. 特征降维 + 归一化
        feat1 = self.layer_norm1(F.gelu(self.fc_1(feat1)))  # [batch, hidden_dim]
        feat2 = self.layer_norm1(F.gelu(self.fc_2(feat2)))  # [batch, hidden_dim]
        feat3 = self.layer_norm1(F.gelu(self.fc_3(feat3)))  # [batch, hidden_dim]

        # 3. 注意力加权
        # 堆叠特征: [batch, 3, hidden_dim]
        stacked_feats = torch.stack([feat1, feat2, feat3], dim=1)

        # 应用注意力机制 - 自注意力
        attended_feats, attention_weights = self.attention_weights(
            stacked_feats, stacked_feats, stacked_feats
        )
        # attended_feats: [batch, 3, hidden_dim]
        # attention_weights: [batch, 3, 3] - 注意力权重矩阵

        # 4. 加权特征融合
        # 使用注意力权重对原始特征进行加权
        # attention_weights形状: [batch, 3, 3]，最后一个维度是每个位置对其他位置的注意力
        # 我们取每个位置对自己的注意力作为权重（对角线元素）
        self_attention_weights = attention_weights.diagonal(dim1=1, dim2=2)  # [batch, 3]

        # 对权重进行softmax，确保和为1
        normalized_weights = F.softmax(self_attention_weights, dim=1)  # [batch, 3]

        # 加权求和
        weighted_feat1 = normalized_weights[:, 0].unsqueeze(1) * feat1  # [batch, hidden_dim]
        weighted_feat2 = normalized_weights[:, 1].unsqueeze(1) * feat2  # [batch, hidden_dim]
        weighted_feat3 = normalized_weights[:, 2].unsqueeze(1) * feat3  # [batch, hidden_dim]

        # 5. MLP深度融合
        # 拼接加权后的特征
        concat_weighted = torch.cat([weighted_feat1, weighted_feat2, weighted_feat3], dim=1)  # [batch, hidden_dim*3]

        # 通过MLP进行深度融合
        mlp_fused = self.mlp_fusion(concat_weighted)  # [batch, hidden_dim*3]
        mlp_fused = self.layer_norm2(mlp_fused)

        # 6. 分类
        logits = self.classifier(mlp_fused)  # [batch, num_labels]

        # 计算损失
        if labels is not None:
            loss = nn.CrossEntropyLoss()(logits, labels)
            if return_weights:
                return loss, logits, normalized_weights
            return loss, logits
        else:
            if return_weights:
                return None, logits, normalized_weights
            return None, logits
...
for epoch in range(epochs):
    ...
    if dev_f1 > best_accuracy:
        ...
        torch.save(model.state_dict(), 'sare_model_chem.pth')
...
csv_filename = 'training_log_chem.csv'
...
# 加载最佳模型进行测试
model.load_state_dict(torch.load('sare_model_chem.pth'))
model.to(device)
