...
class SingleBERTModel(nn.Module):
    ...
    def __init__(self, model_path, model_name, num_labels=6, dropout_rate=0.3):
...
def train_single_model(model, train_loader, dev_loader, test_loader, epochs=10,
                       learning_rate=2e-5, device='cuda', save_dir='models'):
    """
    训练单个模型 - 简化版本：只保存CSV训练日志
    """
    model.to(device)
    model.train()
    ...
    for epoch in range(epochs):
        ...
        # ========== 测试阶段 ==========
        test_results = evaluate_single_model(model, test_loader, device, model.model_name)
        test_accuracy = test_results['accuracy']
        test_precision = test_results['precision']
        test_recall = test_results['recall']
        test_f1 = test_results['f1_score']
        avg_test_loss = test_results['loss']
        ...
        with open(log_file, 'a', newline='', encoding='utf-8-sig') as f:
            ...
            writer.writerow(
                [epoch + 1, 'test', avg_test_loss, test_accuracy, test_precision, test_recall, test_f1, timestamp])
        ...
        print(f"测试集 - Loss: {avg_test_loss:.4f}, Accuracy: {test_accuracy:.4f}, F1: {test_f1:.4f}")
    ...
    for model_name, model_path in model_configs.items():
        ...
        if train_single:
            best_f1 = train_single_model(
                ...
                learning_rate=2e-5,
                device=device,
                save_dir='baseline_chem_models'
            )

            # 加载最佳权重
            model_save_dir = os.path.join('baseline_chem_models', model_name)
            best_model_path = os.path.join(model_save_dir, 'best_model.pth')
...
if __name__ == "__main__":
    ...
    label_map = {'CPR:3': 0, 'CPR:9': 1, 'CPR:5': 2, 'CPR:6': 3, 'CPR:4': 4, 'false': 5}
